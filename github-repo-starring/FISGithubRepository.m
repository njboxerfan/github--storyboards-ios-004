...
- (NSString *)name
{
    return [self.fullName componentsSeparatedByString:@"/"][1];
}

- (void)setName:(NSString *)name
{
    NSString *pre = [self.fullName componentsSeparatedByString:@"/"][0];
    self.fullName = [NSString stringWithFormat:@"%@/%@",pre,name];
}
@end
